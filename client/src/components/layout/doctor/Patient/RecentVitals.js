...
const RecentVitals = ({ checkinId, showTotal = 3 }) => {
  ...
  useEffect(() => {
    ...
    axios.get(`/api/doctor/data/vitals?checkinId=${checkinId}&showTotal=${showTotal}`)
    ...
    <div className="card card-outline card-danger">
      <div className="card-header">
        ...
        <div className="card-tools">
          <Link to={`/doctor/dashboard/reportVitals/${checkinId}`} className="bg-red btn btn-sm">
            Record Vitals
          </Link>
        </div>
      </div>
      ...
      <div className="card-footer">
        <Link to={`/doctor/dashboard/allVitals/${checkinId}`} className="float-right btn btn-primary btn-sm">
          View All
        </Link>
      </div>
    </div>
