...
const HealthTimeline = ({ checkinId }) => {
  ...
  useEffect(() => {
    axios.get(`/api/nurse/data/patientCheckins?checkinId=${checkinId}`)
      .then(res => {
    ...
    <div className="card card-primary">
      ...
      <div className="card-body" style={{ height: '812px' }} >
        <ul className="px-5 timeline" style={{ overflowY: 'auto', maxHeight: '750px' }}>
          {patientCheckins && patientCheckins.map((checkin, index) => (
            <li key={checkin._id}>
              {
              ...
              <p className="mt-1 text-gray text-bold text-yellow">
                {index === 0 && <span className="p-1 badge badge-warning">Current Checkin</span>}
              </p>
              <p className="mb-2">
              ...
              <div className="mt-4 mb-4">
                <Link
                  to={`/nurse/dashboard/detailedCheckin/${checkinId}`}
