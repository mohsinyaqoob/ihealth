...
const RevealAssignedLabTests = props => {
  ...
  useEffect(() => {
    ...
    axios.get(`/api/lab/data/assignedLabTests?patient=${patientId}`)
      .then(res => {
        setAssignedLabTests(res.data.assignedLabTests)
        ...
        <div className="container-fluid">
          <div className="mb-2 row">
            <div className="col-sm-6">
              <h1 className="m-0">Assigned Lab Tests</h1>
            </div>{/* /.col */}
            <div className="col-sm-6">
              <ol className="breadcrumb float-sm-right">
                <li className="breadcrumb-item">Laboratory</li>
                <li className="breadcrumb-item">Dashboard</li>
                <li className="breadcrumb-item">Assigned Lab Tests</li>
              </ol>
            </div>{/* /.col */}
          </div>{/* /.row */}
        </div>{/* /.container-fluid */}
                    ...
                    <tbody>
                      {
                        assignedLabTests.map(test => (
                          <tr key={test._id}>
                            ...
                            <td>
                              {
                                test.testStatus === 1 ? <Link to={`/lab/dashboard/uploadLabReport/${test.patient}/${test._id}`} className="btn btn-primary btn-sm">Upload Report</Link> : <a href={test.report} target="_blank" className="btn btn-success btn-sm">Reveal Report</a>
                              }
                            </td>
                          </tr>
                        ))
...
export default RevealAssignedLabTests
